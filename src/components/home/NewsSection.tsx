...
export default function NewsSection() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await articlesApi.getPublished(1, 4);
        if (response.success && response.data && response.data.length > 0) {
          setArticles(response.data);
        } else {
          const directResponse = await articlesApi.getPublished(1, 100);
        ...
        <div className="container mx-auto">
          ...
          <div className="hidden lg:flex flex-wrap gap-8 justify-center">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="flex flex-col h-full lg:shadow-xl lg:rounded-3xl lg:p-4 bg-[#F1FAFF] animate-pulse w-full sm:w-[calc(50%-1rem)] lg:w-[calc(25%-1.5rem)] max-w-sm"
              >
      ...
      <div className="container mx-auto">
                ...
                {articles.map((article) => (
                  <div key={article.id} className="w-1/2 flex-shrink-0 px-4">
                    <div className="flex flex-col h-full">
                      ...
                      />
                      <div className="flex flex-col gap-3 mt-4 flex-1">
                        <h2 className="text-lg font-plant text-primary">
        ...
        <div className="hidden lg:flex flex-wrap gap-8 justify-center">
          {articles.map((article) => (
            <div
              key={article.id}
              className="flex flex-col h-full lg:shadow-xl lg:rounded-3xl lg:p-4 bg-[#F1FAFF] hover:scale-105 transition-transform duration-300 w-full sm:w-[calc(50%-1rem)] lg:w-[calc(25%-1.5rem)] max-w-sm"
            >
              <Image
                src={article.featured_image_url}
                alt={article.title}
        ...
        <div className="hidden md:flex justify-center mt-16 lg:mt-28">
          <Link
            href="/articles"
            className="text-primary font-plant text-2xl lg:text-3xl flex items-center gap-2"
          >
            {t("home.news_section_cta")}
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
